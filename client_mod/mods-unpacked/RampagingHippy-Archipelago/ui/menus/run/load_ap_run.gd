...
func _ready():
	...
	if player_character == null:
		return
	_saved_run = _ap_client.saved_runs_progress.get_saved_run(player_character.my_id)
	
	var loader_v2 = ProgressDataLoaderV2.new("")
	var deserialized_run = loader_v2.deserialize_run_state(_saved_run)
	var player_data = deserialized_run["players_data"][0]
	...
	_background.texture = ZoneService.get_zone_data(deserialized_run.current_zone).ui_background
