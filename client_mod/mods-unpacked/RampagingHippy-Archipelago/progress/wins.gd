...
func _init(ap_session, game_state).(ap_session):
	...
	_game_state.connect("run_finished", self, "_on_run_finished")
...
func _on_run_finished(won_run: bool, character_id: String):
	if won_run:
		...
		_ap_session.check_location(character_won_loc_id)

func on_item_received(item_name: String, _item):
	if item_name == "Run Won":
		num_wins += 1
		if num_wins >= wins_for_goal:
			_ap_session.set_status(ApTypes.ClientStatus.CLIENT_GOAL)

func on_connected_to_multiworld():
	wins_for_goal = _ap_session.slot_data["num_wins_needed"]
