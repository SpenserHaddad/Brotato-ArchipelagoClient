...
func _init(ap_client, game_state).(ap_client, game_state):
	...
	var _status = _ap_client.connect(
		"data_storage_updated", self, "_on_session_data_storage_updated"
	)
...
func on_wave_finished(wave_number: int, character_ids: Array, _is_run_lost: bool, _is_run_won: bool):
	...
	for character_id in character_ids:
		...
		if character in updated_run_progress:
			# This can happen if multiple players pick the same character in co-op.
			continue
...
func _on_session_data_storage_updated(key: String, new_value, _original_value = null):
	if key == _character_run_progress_slot_data_key:
		for character in character_info:
			character_info[character].max_wave_completed = new_value[character]["max_wave_completed"]
