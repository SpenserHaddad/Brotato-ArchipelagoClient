...
- Fixed the client failing to connect if the player's name had non-ASCII characters.
...
- Added Wounded character from All Pain No Gain update (Brotato 1.1.15.0).
...
- Fixed the game disconnecting when DeathLink is enabled upon receiving non-DeathLink
  Bounced commands (which appeared as the game disconnecting after a short time).
- Fixed DeathLink adding the room's tags to the player slot.
- Fixed template YAML requiring unreleased AP version.
...
[unreleased]: https://github.com/SpenserHaddad/Brotato-ArchipelagoClient/compare/v0.15.1...HEAD
